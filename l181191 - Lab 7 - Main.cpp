...
bankAccount**ptrarr( string name, int& size)
{
	int x;
	ifstream fin(name);
	fin >> x;
	size =x;
	bankAccount**arr = new bankAccount* [x];
	...
	for (int i=0; i<x; i++)
	{
		...
		float *transactions = new float [no];
		...
		for (int j=0; j<x; j++)
		{
			arr[j]= new bankAccount(id, balance , transactions, no, limit);
...
//
	...
	arr[0] = new bankAccount (30,2000,transactions,3, 100000);
...
//	
	...
	bankAccount ** darr = new bankAccount*[2];
	float trans[2] = {-5000,5000};

	darr[0] = new bankAccount (50, 2000, trans,2,100000);
	darr[1] = new bankAccount(60,6000,400000);
	darr[0]-> withdraw(1000);
	darr[0]-> print();
	darr[1] -> print();
//
//
...
//	To return any array of pointer of object frotm funcion return type should be 
//	<className> **
...
//	*/
//	printacc(ptrarr(name,size),size);
////	
//
//	/********************************TASK 8*************************************
